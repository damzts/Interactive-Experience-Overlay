...
const widgetManifest: Record<RegisteredWidgetComponentType, WidgetManifestEntry> = {
  ...
  'media-deck':          { load: () => import('./MediaDeckWidget').then((m) => m.MediaDeckWidget),                   defaultPosition: { x: 188,  y: 102 }, defaultSize: { width: 320              } },
  'music':               { load: () => import('./MusicWidget').then((m) => m.MusicWidget),                           defaultPosition: { x: 60,   y: 120 }, defaultSize: { width: 280              } },
