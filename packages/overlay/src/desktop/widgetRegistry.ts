...
const widgetManifest: Record<RegisteredWidgetComponentType, () => Promise<DesktopWidgetRenderer>> = {
  ...
  'online-stream':       () => import('./POVStreamWidget').then((m) => m.POVStreamWidget),
  'pov-stream':          () => import('./POVStreamWidget').then((m) => m.POVStreamWidget),
  'participant-stream':  () => import('./ParticipantStreamWidget').then((m) => m.ParticipantStreamWidget),
