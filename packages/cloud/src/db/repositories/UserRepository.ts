...
function appendSlugSuffix(baseSlug: string): string {
  ...
  let trimmed = baseSlug.slice(0, maxBase).replace(/-+$/, '')
...
export class UserRepository {
  ...
  async findByGoogleId(googleId: string): Promise<UserRecord | null> {
    const result = await this.pool.query<UserRow>('SELECT * FROM users WHERE google_id = $1', [googleId])
    return result.rows[0] ? rowToRecord(result.rows[0]) : null
  }

  async findBySlug(slug: string): Promise<UserRecord | null> {
    const result = await this.pool.query<UserRow>('SELECT * FROM users WHERE slug = $1', [slug])
    return result.rows[0] ? rowToRecord(result.rows[0]) : null
  }
  ...
  async upsertFromGoogle(profile: GoogleProfile): Promise<UserRecord> {
    ...
    if (existing) {
      const result = await this.pool.query<UserRow>(
        `UPDATE users SET display_name = $1, avatar_url = $2, updated_at = NOW() WHERE google_id = $3 RETURNING *`,
    ...
    while (retries < MAX_SLUG_RETRIES) {
      try {
        const result = await this.pool.query<UserRow>(
          `INSERT INTO users (google_id, email, display_name, avatar_url, slug) VALUES ($1, $2, $3, $4, $5) RETURNING *`,
  ...
  async getRefreshTokenHash(userId: string): Promise<string | null> {
    const result = await this.pool.query<{ refresh_token_hash: string | null }>('SELECT refresh_token_hash FROM users WHERE id = $1', [userId])
