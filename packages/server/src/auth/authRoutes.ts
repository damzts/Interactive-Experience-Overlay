 ...
 * Registers: GET /auth/google, GET /auth/google/callback, POST /auth/refresh, POST /auth/logout, GET /auth/me
 */
export async function authRoutes(app: FastifyInstance, opts: AuthRoutesOptions): Promise<void> {
  ...
  // GET /auth/google/callback — exchange code, upsert user, issue cookies, redirect to admin
  ...
  // POST /auth/refresh — validate refresh token from cookie, issue new cookies
  app.post(
    '/auth/refresh',
    async (request, reply) => {
  ...
  // POST /auth/logout — invalidate refresh token, clear all cookies
