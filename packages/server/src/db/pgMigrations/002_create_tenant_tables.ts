/**
 * Migration 002: Create all tenant-scoped tables.
 * Each table has a user_id FK referencing users(id) with ON DELETE CASCADE.
 * Tables use composite primary keys (user_id, id) or user_id as sole PK for single-row-per-user tables.
 */
...
export const migration002: Migration = {
  ...
  up: async (client: PoolClient) => {
    // scenes — composite PK (user_id, id)
    await client.query(`
      CREATE TABLE scenes (
        id TEXT NOT NULL,
    ...
    // applications — composite PK (user_id, id)
    await client.query(`
    ...
    // desktop_config — single row per user, user_id as PK
    await client.query(`
    ...
    // widget_layouts — composite PK (user_id, id)
    await client.query(`
    ...
    // events — composite PK (user_id, id)
    await client.query(`
      CREATE TABLE events (
        id TEXT NOT NULL,
        user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    ...
    // keybinds — composite PK (user_id, scope, key)
    await client.query(`
    ...
    // obs_config — single row per user, user_id as PK
    await client.query(`
    ...
    // audio_config — single row per user, user_id as PK
    await client.query(`
    ...
    // overlay_style — single row per user, user_id as PK
    await client.query(`
    ...
    // desktop_ambiance — single row per user, user_id as PK
    await client.query(`
    ...
    // source_presets — composite PK (user_id, id)
    await client.query(`
    ...
    // media_library — composite PK (user_id, id)
    await client.query(`
    ...
    // pov_config — single row per user, user_id as PK
    await client.query(`
    ...
    // pov_feeds — composite PK (user_id, id)
    await client.query(`
    ...
    // online_config — single row per user, user_id as PK
    await client.query(`
