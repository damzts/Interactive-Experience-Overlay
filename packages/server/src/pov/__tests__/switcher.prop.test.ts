...
describe('POVSwitcher - Automatic Switching Property Tests (switcher.prop)', () => {
  afterEach(() => {
    vi.useRealTimers()
  })
  ...
  describe('Property 10: Automatic switching threshold decision', () => {
    ...
    it('no switch occurs when no candidate exceeds threshold', () => {
      fc.assert(
        fc.property(
          fc.double({ min: 0.05, max: 0.5, noNaN: true }),  // activityThreshold
          fc.double({ min: 0.3, max: 0.9, noNaN: true }),   // currentScore
          fc.integer({ min: 2, max: 6 }),                    // numFeeds
          (activityThreshold, currentScore, numFeeds) => {
            const registry = createRegistry()
            const switcher = new POVSwitcher(registry, {
              cooldownMs: 1000,
              activityThreshold,
            ...
            // Set first feed as active with currentScore
            const initialScores = new Map<string, number>()
            initialScores.set(feedIds[0], currentScore)
            ...
            // All candidates below threshold difference
            const scores = new Map<string, number>()
            scores.set(feedIds[0], currentScore)
            for (let i = 1; i < feedIds.length; i++) {
              // Each candidate is below current + threshold
              const candidateScore = currentScore + activityThreshold * 0.5 * (i / numFeeds)
              scores.set(feedIds[i], Math.min(candidateScore, 1.0))
            }
            switcher.evaluateScores(scores)
  ...
  describe('Property 11: Silence threshold prevents switching', () => {
    it('no switch occurs when all scores are below silenceThreshold', () => {
      fc.assert(
        fc.property(
          fc.double({ min: 0.1, max: 1.0, noNaN: true }), // silenceThreshold
          ...
          (silenceThreshold, numFeeds) => {
            ...
            // All scores strictly below silence threshold
    ...
    it('silence threshold applies even when score differences exceed activityThreshold', () => {
      fc.assert(
        fc.property(
          fc.double({ min: 0.2, max: 0.8, noNaN: true }), // silenceThreshold
          (silenceThreshold) => {
            const registry = createRegistry()
            const switcher = new POVSwitcher(registry, {
              cooldownMs: 1000,
              activityThreshold: 0.01, // very low threshold
              silenceThreshold,
            })
            ...
            // Set feedA as active with score above silence threshold
            const initialScores = new Map<string, number>([
              [feedA, silenceThreshold + 0.1],
              [feedB, 0],
            ])
            switcher.evaluateScores(initialScores)
            ...
            // Both below silence threshold, but feedB has a large difference from feedA
            const scoreA = silenceThreshold * 0.1
            const scoreB = silenceThreshold * 0.9 // large diff but still below silence
            fc.pre(scoreB < silenceThreshold)
            fc.pre(scoreB - scoreA > 0.01) // exceeds activityThreshold
            ...
            // Should NOT switch because all scores are below silence threshold
            expect(switcher.activeCameraId).toBe(feedA)
          },
        ),
        { numRuns: 100 },
   ...
   *
   * No switch occurs within cooldownMs of the last switch.
   */
  ...
  describe('Property 13: Initial camera selection without constraints', () => {
    ...
    it('initial selection ignores activityThreshold even for low scores', () => {
      fc.assert(
        fc.property(
          fc.double({ min: 0.001, max: 0.05, noNaN: true }), // very low score
          fc.double({ min: 0.5, max: 1.0, noNaN: true }),     // high activityThreshold
          (lowScore, activityThreshold) => {
            const registry = createRegistry()
            const switcher = new POVSwitcher(registry, {
              cooldownMs: 30000,
              activityThreshold,
            ...
            const feedId = registerConnectedFeed(registry, 'OnlyFeed', 'ws://only.local:4455')

            const scoreMap = new Map<string, number>([[feedId, lowScore]])
            switcher.evaluateScores(scoreMap)

            // Should select even with very low score (no threshold on initial)
  ...
  describe('Property 14: Tie-breaking preserves current or selects least-recently-active', () => {
      ...
      fc.assert(
        fc.property(
          ...
          (tiedScore, numTied) => {
            ...
            // All feeds tied at the same score — no candidate exceeds current by threshold
            ...
            // Current camera should be retained (tied, diff = 0 < threshold)
            expect(switcher.activeCameraId).toBe(feedIds[0])
          },
        ),
        { numRuns: 100 },
      ...
      fc.assert(
        fc.property(
          ...
          (tiedScore, numTied) => {
            ...
            // Should select one of the tied feeds
    ...
    it('selects least-recently-active among tied candidates that exceed threshold', () => {
      fc.assert(
        fc.property(
          fc.double({ min: 0.5, max: 1.0, noNaN: true }), // tied candidate score
          (tiedCandidateScore) => {
            const registry = createRegistry()
            const switcher = new POVSwitcher(registry, {
              cooldownMs: 1000,
            ...
            const feedA = registerConnectedFeed(registry, 'FeedA', 'ws://feedA.local:4455')
            const feedB = registerConnectedFeed(registry, 'FeedB', 'ws://feedB.local:4455')
            const feedC = registerConnectedFeed(registry, 'FeedC', 'ws://feedC.local:4455')

            // Make feedA active first
            switcher.evaluateScores(new Map([[feedA, 0.8], [feedB, 0.1], [feedC, 0.1]]))
            expect(switcher.activeCameraId).toBe(feedA)

            vi.advanceTimersByTime(2000)

            // Switch to feedB
            switcher.evaluateScores(new Map([[feedA, 0.1], [feedB, 0.9], [feedC, 0.1]]))
            expect(switcher.activeCameraId).toBe(feedB)

            vi.advanceTimersByTime(2000)

            // Now feedA and feedC are tied and exceed feedB by threshold
            const currentScore = 0.2
            fc.pre(tiedCandidateScore - currentScore >= 0.1)

            switcher.evaluateScores(new Map([
              [feedA, tiedCandidateScore],
              [feedB, currentScore],
              [feedC, tiedCandidateScore],
            ]))

            // feedC was never active (least recently active), feedA was active earlier
            // So feedC should be selected
  ...
  describe('Property 15: Disconnect triggers immediate fallback switch', () => {
    ...
    it('emits switch event with fallback reason on disconnect', () => {
      fc.assert(
        fc.property(
          ...
          (numRemaining) => {
            ...
            for (let i = 0; i < numRemaining; i++) {
              remainingIds.push(registerConnectedFeed(registry, `R${i}`, `ws://r${i}.local:4455`))
              registry.updateActivityScore(remainingIds[i], 0.5)
            }

            // Set active
            switcher.evaluateScores(new Map([[activeFeedId, 1.0], ...remainingIds.map(id => [id, 0] as [string, number])]))
            expect(switcher.activeCameraId).toBe(activeFeedId)

            // Track switch events after initial setup
            const switchEvents: Array<{ prev: string | null; next: string; reason: string }> = []
            switcher.onSwitch((prev, next, _ts, reason) => {
              switchEvents.push({ prev, next, reason })
            })
            ...
            // Should emit a fallback switch event
            expect(switchEvents.length).toBeGreaterThanOrEqual(1)
            const lastEvent = switchEvents[switchEvents.length - 1]
            expect(lastEvent.prev).toBe(activeFeedId)
            expect(lastEvent.reason).toBe('fallback')
            expect(remainingIds).toContain(lastEvent.next)
          },
        ),
        { numRuns: 100 },
      )
