...
/**
 * Generate a list of unique feed IDs with a guaranteed minimum count.
 */
...
describe('TransitionQueue Property Tests', () => {
  ...
  describe('Property 22: Transition queue respects max depth', () => {
    ...
    it('when a 6th item is enqueued, the oldest is discarded and the new one is appended', async () => {
      await fc.assert(
        fc.asyncProperty(
          // Generate exactly 7+ unique feed IDs to overflow the queue
          feedIdListArb(7, 15),
          async (feedIds) => {
            const mockObs = createMockObs()
            const connectedFeeds = new Set(feedIds)
   ...
   *
   * When processing the queue, any request referencing a feed that is no longer
   * connected is skipped/discarded.
   */
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (feedIds) => {
            ...
            // Enqueue first feed — processes immediately
            queue.enqueue(feedIds[0])
            await vi.advanceTimersByTimeAsync(0)
